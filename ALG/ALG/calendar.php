...
// Make sure sched_id is selected so events have an id for delete
$events = [];
$sql = "
    SELECT 
        sc.sched_id,
    ...
    FROM service_sched sc
    LEFT JOIN Services s ON sc.Services_id = s.Services_id
    LEFT JOIN Orders o ON o.Services_id = sc.Services_id
    LEFT JOIN Customers c ON o.Customer_id = c.Customer_id
    ORDER BY sc.ScheduleDate ASC
";
...
<script>
/*
  Single calendar instance, events are provided by PHP ($events).
  Deletion calls delete_event.php with sched_id.
*/

let calendar;              // FullCalendar instance
let currentEvent = null;   // currently clicked event (FullCalendar Event Object)
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    eventClick: function(info) {
      // store reference to event for deletion
    ...
    // events loaded from PHP-generated JSON
    events: <?php echo json_encode($events, JSON_HEX_TAG|JSON_HEX_AMP); ?>
  });

  calendar.render();

  // style header buttons a little (optional)
  setTimeout(function() {
...
function removeEvent() {
  ...
  if (!id) {
    alert('Event has no id to delete.');
    return;
  }
  ...
  .then(txt => {
    const res = txt.trim();
    if (res === 'success') {
      currentEvent.remove(); // remove from UI
      closeModal();
