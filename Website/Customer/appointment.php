<?php
// scheduling.php - Updated to use appointment_sched table
...
try {
    ...
    if (!$conn || $conn->connect_error) {
        throw new Exception("Database connection failed: " . ($conn ? $conn->connect_error : "Unable to connect"));
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['schedule_service'])) {
        ...
        try {
            ...
            $check_schedule_sql = "SELECT sched_id FROM appointment_sched WHERE ScheduleDate = ?";
            ...
            // Insert into appointment_sched table with appointment_type
            $stmt_sched = $conn->prepare("INSERT INTO appointment_sched (ScheduleDate, customer_id, appointment_type) VALUES (?, ?, ?)");
...
<style>
  * {
  ...
  .form-group textarea:focus {
    outline: none;
    border-color: #4f9bd2;
    box-shadow: 0 0 0 3px rgba(79, 155, 210, 0.1);
  }
...
<nav>
  ...
  <div class="nav-links">
      ...
      <a href="scheduling.php" class="active">Scheduling</a>
...
<div class="container">
  ...
  <div class="calendar-header">
    <h1 class="calendar-title">Appointment Schedule</h1>
  ...
  <?php if ($db_error): ?>
    <div class="db-error">
      <strong>Database Connection Error:</strong> <?php echo htmlspecialchars($db_error); ?><br>
...
<div id="eventModal" class="modal">
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <p><em>Please contact the us if this is a mistake.</em></p>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  setTimeout(function() {
    document.querySelectorAll('.fc-button').forEach(btn => {
      btn.style.background = '#042842';
      btn.style.color = '#fff';
      btn.style.borderColor = '#042842';
